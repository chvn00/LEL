# File: per_task_accuracy.py
# Experiment: Final per-task accuracy after the full Split-MNIST sequence
#
# Paper:
...
#
# Purpose:
#   After all five Split-MNIST tasks, measure accuracy on each task separately.
#   Vanilla local learning retains only the most recent task (zero on the rest),
#   whereas generative dreaming retains every task.
#
# Method:
#   - Class-incremental Split-MNIST with the local LEL classifier
#   - Two modes: vanilla and generative dreaming (mode 'gen')
#   - Per-task accuracy evaluated at the end of the sequence
#
# Outputs:
#   - Console: list of per-task accuracies for vanilla and for generative
#   - No files are written (results are printed to the console)
#
# How to run:
#   python3 per_task_accuracy.py
...
def acc_task(W, mu, a, b):
    ...
    return float(np.mean([np.mean(np.argmax(fwd(W, Xte_all[np.where(yte_all == c)[0][:400]] - mu), 1) == c) for c in (a, b)]))


def run(mode, seed=0):
    """Run the full sequence; return the final per-task accuracies."""
    rng = np.random.default_rng(seed)
    Xc = {c: class_data(Xtr_all, ytr_all, c, nper, rng) for c in range(K)}; mu = np.vstack(list(Xc.values())).mean(0); Xc = {c: Xc[c] - mu for c in range(K)}
    gens = {c: g_train(Xc[c], seed=seed + c) for c in range(K)} if mode == 'gen' else {}
    W = (rng.normal(0, 1, (H, 784)) * np.sqrt(1 / 784), np.zeros((1, H)), rng.normal(0, 1, (K, H)) * np.sqrt(1 / H), np.zeros((1, K))); seen = []
    for t in range(5):
    ...
    return [acc_task(W, mu, 2 * t, 2 * t + 1) for t in range(5)]


for m in ['vanilla', 'gen']:
    print(m, ["%.3f" % x for x in run(m)])
